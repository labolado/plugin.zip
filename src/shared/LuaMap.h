	...
	public:
		LMap(lua_State *L, int t)
		{
			...
			while ( lua_next( L, t ) != 0 )
			{
				...
				{
					// Here we manually convert the value to a string, rather than using lua_tostring.
					// This is because lua_tolstring (via lua_tostring) converts the actual value on
					// the stack to a string, which we do not want to do when using lua_next.
					std::stringstream ss;
					ss << lua_tonumber( L, -2 );
					key = ss.str();
				}
				else if (LUA_TSTRING == kType)
				{
					key = lua_tostring( L, -2 );
				}
				else
				{
					// We got a key that isn't a number or string.
					// As we don't know how to handle this, we ignore the key/value.
					lua_pop( L, 1 );
					continue;
				}
